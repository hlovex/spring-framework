...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	public Set<BeanDefinition> findCandidateComponents(String basePackage) {
		if (this.componentsIndex != null && indexSupportsIncludeFilters()) {
			// 可以在 resources/META-INF/spring.components 文件中直接定义某个类的索引文件
			// 如 com.xxx.UserService=org.springframework.stereotype.Component
		...
		try {
			...
			for (Resource resource : resources) {
				...
				try {
					...
					// 排除 excludeFilters 选项，扫描 includeFilters 选项，
					// 之所以能够扫描 @Component 是因为在 includeFilters 里面默认添加了该选项
					// this.includeFilters.add(new AnnotationTypeFilter(Component.class));
					if (isCandidateComponent(metadataReader)) {
						ScannedGenericBeanDefinition sbd = new ScannedGenericBeanDefinition(metadataReader);
	...
	protected boolean isCandidateComponent(AnnotatedBeanDefinition beanDefinition) {
		...
		// isIndependent 独立的，当实例化普通内部类的时候，需要依赖外部类，所以普通内部类不能成为一个 bean
		// isConcrete 接口和抽象类也不能成为 bean，但是当抽象类里面有添加了 @Lookup 注解的方法时候可以
